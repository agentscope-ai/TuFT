...
def patch_vllm_prompt_logprobs(model_runner: GPUModelRunner):  # noqa: C901
    ...
    if version < parse_version("0.10.2") or version > parse_version("0.23.0"):
        raise ValueError(
            f"Unsupported vllm version: {vllm.__version__}. "
            "This patch requires vllm version >= 0.10.2, <= 0.23.0."
        )
    ...
    def _get_prompt_logprobs_dict_v23(
        self,
        hidden_states: torch.Tensor,
        num_scheduled_tokens: dict[str, int],
    ) -> dict[str, Optional[LogprobsTensors]]:
        """Patched version of _get_prompt_logprobs_dict for vLLM >= 0.20.0.

        Changes from v12: in_progress_prompt_logprobs_cpu is per-request,
        gather_logprobs returns 4 values, cleanup sets attribute to None.
        """
        num_prompt_logprobs_dict = self.num_prompt_logprobs
        if not num_prompt_logprobs_dict:
            return {}
        ...
        for req_id, num_prompt_logprobs in num_prompt_logprobs_dict.items():
            ...
            # PATCH: apply temperature scaling
            temp = request.sampling_params.temperature
            if temp >= 1e-5:
                logits.div_(temp)
            # END PATCH
    ...
    elif get_vllm_version() < parse_version("0.20.0"):
        model_runner._get_prompt_logprobs_dict = MethodType(
            _get_prompt_logprobs_dict_v12, model_runner
        )
    else:
        model_runner._get_prompt_logprobs_dict = MethodType(
            _get_prompt_logprobs_dict_v23, model_runner
        )
