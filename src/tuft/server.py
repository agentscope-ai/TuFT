    ...
    async def save_weights_for_sampler(
        ...
        async def _operation() -> types.SaveWeightsForSamplerResponseInternal:
            ...
            if request.sampling_session_seq_id is not None:
                # `save_weights_and_get_sampling_client` is a combined
                # operation. Reject a training-only model before writing a
                # checkpoint so a disabled sampling capability cannot leave a
                # partial side effect behind.
                training_run = state.get_training_run_record(request.model_id, user.user_id)
                state.sampling.require_sampling_capability(training_run.base_model)
