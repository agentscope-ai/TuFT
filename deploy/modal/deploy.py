...
Usage:
    ...
    python deploy/modal/deploy.py --config my.yaml

    # or override per-run with flags:
    python deploy/modal/deploy.py --config my.yaml --gpu L4 --serve          # ephemeral
    python deploy/modal/deploy.py --config my.yaml --gpu H100:2 --hf-secret huggingface --min-containers 1
...
def load_config(path: Path) -> dict:
    ...
    if bad:
        sys.exit(f"config error: authorized_users keys must start with 'tml-' "
...
def embed_config(path: Path, data: dict) -> str:
    ...
    if data and "modal" in data:
        import yaml
        clean = {k: v for k, v in data.items() if k != "modal"}
...
def render_app(infra: dict, config_b64: str) -> str:
    header = "\n".join([
        "# AUTO-GENERATED by deploy/modal/deploy.py — DO NOT EDIT.",
        "# Edit your tuft_config.yaml and re-run deploy.py instead.",
...
def main() -> None:
    ...
    ap.add_argument("--min-containers", dest="min_containers", type=int, default=None,
                    help="1 keeps it warm (no scale-to-zero) — useful for long laptop-driven loops")
    ap.add_argument("--max-containers", dest="max_containers", type=int, default=None,
                    help="keep at 1 — TuFT state lives in one container")
    ap.add_argument("--max-concurrent", dest="max_concurrent", type=int, default=None)
    ap.add_argument("--proxy-auth", dest="proxy_auth", action="store_const", const=True, default=None,
                    help="add Modal gateway auth IN FRONT of X-API-Key (breaks the plain Tinker SDK)")
    ap.add_argument("--no-proxy-auth", dest="proxy_auth", action="store_const", const=False,
                    help="force proxy auth off (overrides a `modal:` section)")
    ap.add_argument("--hf-secret", dest="hf_secret", default=None,
    ...
    ap.add_argument("--registry-secret", dest="registry_secret", default=None,
                    help="Modal secret name for a PRIVATE image registry (REGISTRY_USERNAME/PASSWORD)")
    ap.add_argument("--serve", "--foreground", dest="serve", action="store_true",
                    help="FOREGROUND/ephemeral (modal serve): stays up while running, Ctrl-C tears it down")
    ap.add_argument("--down", "--stop", dest="down", action="store_true",
                    help="DETACHED shutdown: stop a deployed app (modal app stop <name>) and exit")
    ap.add_argument("--dry-run", action="store_true", help="render/print the command but do not run it")
    args = ap.parse_args()
    ...
    if args.down:
        ...
        if rc != 0:
            print("Could not stop it — run `modal app list` to find the exact app name/id.",
                  file=sys.stderr)
        sys.exit(rc)
    ...
    print(f"[render] {GENERATED_APP.name}  (app={infra['name']} gpu={infra['gpu']} "
          f"proxy_auth={infra['proxy_auth']} min/max={infra['min_containers']}/{infra['max_containers']})")
    ...
    if rc == 0 and not args.serve:
        ...
        print("   tinker.ServiceClient(base_url='<url>', api_key='tml-...')  # your authorized_users key")
    sys.exit(rc)
