...
class MathAgentTask(AgentTask):
    ...
    def step(self, action_text: str, metadata: Dict[str, Any]) -> StepResult:
        ...
        if not match:
            return StepResult(
                observation="Invalid action format. Use: Action: Calculate[expression] or Action: Finish[number]",
                reward=0.0,
                done=False,
    ...
    def _do_calculate(self, expression: str) -> StepResult:
        ...
        if result is None:
            return StepResult(
                observation=f"Error: Cannot evaluate '{expression}'. Use valid arithmetic (+, -, *, /, **, parentheses).",
                reward=0.0,
                done=False,
