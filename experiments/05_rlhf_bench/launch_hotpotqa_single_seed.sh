#!/usr/bin/env bash
# HotpotQA GRPO single-seed benchmark (agent task)
set -euo pipefail
...
EVAL_INTERVAL=10
EVAL_N=30
GRPO_G=4      # fewer episodes per step (each episode is multi-turn)
TEMP=0.7
LORA_RANK=8
MAX_TOKENS=256
...
SEED=42
OUTDIR="./results/bench_hotpotqa_seed${SEED}"
mkdir -p "${OUTDIR}"
echo "===== HotpotQA Seed ${SEED} -> ${OUTDIR} ====="
