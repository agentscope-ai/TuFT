...
def experiment3_is_weight_reconstruction(
    ...
    log_is_per_seq = np.array([0.0 for _ in deltas_per_seq])  # oracle per-seq → sum = 0
    ...
    print(
        f"  → But {mean_resid_mae / mean_raw_mae * 100:.1f}% of per-token MAE remains as irreducible noise"
    )
    ...
    # Add value labels
    for bar, val in zip(bars, is_errors):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
