...
def main() -> None:
    ...
    print("\n[summary]")
    print(
        f"  Token MAE:    {summ.baseline_token_mae:.5f} → {summ.corrected_token_mae:.5f}  ({mae_impr:+.1f}% improvement)"
    )
    print(
        f"  Token bias:   {summ.baseline_token_bias:.5f} → {summ.corrected_token_bias:.5f}  ({bias_impr:+.1f}% improvement)"
    )
