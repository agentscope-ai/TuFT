...
from .tasks.base import Task, TaskPrompt
...
class Tenant:
    ...
    async def _run_agent_episode(self) -> None:
        ...
        # Each turn gets the same episode reward (credit assignment handled by advantage normalization)  # noqa: E501
        for turn_data in episode_turns:
            self.buffer.append(
                BufferItem(
                    prompt_tokens=turn_data["prompt_tokens"],
    ...
    async def _do_train(self, batch: List[BufferItem]) -> None:
        ...
        try:
            ...
            datums = []
            for item, adv in zip(batch, advantages, strict=False):
                datums.append(
                    {
                        "prompt_tokens": item.prompt_tokens,
                        "response_tokens": item.response_tokens,
            ...
            if training_lps and len(training_lps) == len(batch):
                ...
                if self._logprob_dump_writer is not None:
                    for idx, (item, samp, trn) in enumerate(
                        zip(batch, sampling_lps, training_lps, strict=False)
                    ):  # noqa: E501
                        record = {
            ...
            logger.info(
                ...
                f"mean_reward={mean_r:.4f}, mean_staleness={sum(stalenesses) / len(stalenesses):.2f}"  # noqa: E501
            )
