...
class TinkerBackend(TrainingBackend):
    ...
    async def compute_training_logprobs(
        ...
        out: List[List[float]] = []
        for (start, end), loss_out in zip(slices, fwd_result.loss_fn_outputs, strict=False):
    ...
    def _build_tinker_datum(self, datum_dict: Dict[str, Any]) -> types.Datum:
        ...
        assert len(padded_advantages) == expected_len, (
            f"padded_advantages length {len(padded_advantages)} != model_input length {expected_len}"  # noqa: E501
        )
