...
class HotpotQATask(AgentTask):
    ...
    def step(self, action_text: str, metadata: Dict[str, Any]) -> StepResult:
        ...
        if not match:
            return StepResult(
                observation="Invalid action format. Use: Action: Search[query], Lookup[term], or Finish[answer]",  # noqa: E501
                reward=0.0,
                done=False,
    ...
    def _do_search(self, query: str) -> StepResult:
        ...
        self._current_page = None
        return StepResult(
            observation=f"No results found for '{query}'. Try searching for one of: {', '.join(available)}",  # noqa: E501
            reward=0.0,
            done=False,
