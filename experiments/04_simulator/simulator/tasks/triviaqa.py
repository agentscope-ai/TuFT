...
class TriviaQATask(AgentTask):
    ...
    def load_dataset(self) -> None:
        ...
        for row in ds:
            ...
            if search_results:
                ...
                for title, snippet in zip(sr_titles, sr_snippets, strict=False):
    ...
    def step(self, action_text: str, metadata: Dict[str, Any]) -> StepResult:
        ...
        if not match:
            return StepResult(
                observation="Invalid action format. Use: Action: Search[query] or Action: Finish[answer]",  # noqa: E501
                reward=0.0,
                done=False,
    ...
    def _do_search(self, query: str) -> StepResult:
        ...
        for _title, content in self._current_evidence.items():
