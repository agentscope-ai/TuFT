...
def experiment1_length_bin(
    ...
    for m in method_names:
        ...
        for bi, (_lo, _hi) in enumerate(bins):
            mask = bin_ids == bi
            n = int(mask.sum())
            if n == 0:
    ...
    for m in method_names:
        ...
        [bin_stats[m][bi]["n"] for bi in range(len(bins))]
...
def main() -> None:
    ...
    print(
        f"[summary] raw clip: pos={pos_raw:.4f}  neg={neg_raw:.4f}  ratio={pos_raw / max(neg_raw, 1e-9):.2f}x"  # noqa: E501
    )
    if "ml_predictor" in corrections:
        ...
        print(
            f"[summary] ml   clip: pos={pos_ml:.4f}  neg={neg_ml:.4f}  ratio={pos_ml / max(neg_ml, 1e-9):.2f}x"  # noqa: E501
        )
