...
默认情况下（`--torch-backend auto`），安装器会**在下载任何内容之前**检测 NVIDIA 驱动，为固定版本的 torch/vLLM 依赖栈选择兼容的 CUDA wheel 变体（`cu130`，旧的 12.9 驱动则使用 `cu129`），并在安装完成后运行 CUDA 冒烟测试。如果驱动没有兼容的变体，安装器会给出明确的指引并失败，而不是装出一个无法运行的环境。也可以显式指定后端，例如在没有 GPU 的机器上构建镜像时：
