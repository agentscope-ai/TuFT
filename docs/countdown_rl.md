...
This tutorial demonstrates **reinforcement learning (RL)** fine-tuning on the **Countdown** dataset using a **running TuFT server**. Full runnable code is in [chat_sft.ipynb](../examples/countdown_rl.ipynb).
...
3. [Key Concepts](#3-key-concepts)  
   - [Prompting for Verifiable Outputs](#41-prompting-for-verifiable-outputs)  
   - [Reward Design: Format, Validity, Correctness, Shaping](#42-reward-design-format-validity-correctness-shaping)  
   - [Group Sampling and Normalized Advantages (GRPO-like)](#43-group-sampling-and-normalized-advantages-grpo-like)  
   - [TuFT Datum Format for RL (Importance Sampling)](#44-tuft-datum-format-for-rl-importance-sampling)  
   - [loss_fn, Metrics, and EMA Evaluation](#45-loss_fn-metrics-and-ema-evaluation)  
...
This notebook uses **[`Jiayi-Pan/Countdown-Tasks-3to4`](https://huggingface.co/datasets/Jiayi-Pan/Countdown-Tasks-3to4)**.
...
Unlike the Tinker SDK, TuFT must run on local GPUs; the experiments below were conducted on a local 2× NVIDIA A100-SXM4-80GB setup (Driver 550.54.15, CUDA 12.9). Before running the example, follow [README.md](../README.md) to start the TuFT server locally.

Key TuFT calls (full code in [chat_sft.ipynb](../examples/countdown_rl.ipynb)):
```python
import tinker
...
RL works best when the reward signal is reliable. For Countdown, the notebook enforces a **verifiable output contract**:
...
- If not exact:
  ...
  - or use **continuous shaping**, e.g.: r = r_fmt + (1 - r_fmt) * 1 / (1 + |y - target|)
...
### 3.3 Group Sampling and Within-Group Normalized Advantages (GRPO-like)

Instead of sampling a single completion per prompt, the notebook samples a **group** of completions:
...
- For sample i: `A_i = (r_i - mu) / (sigma + eps)`
...
### 3.4 TuFT Datum Format for RL (Importance Sampling)

Each sampled rollout is converted into a TuFT `Datum` that contains:
...
This section explains how to choose `lora_rank` and `learning_rate` for the Countdown RL task, and summarizes conclusions based on the provided experiment results. The following experiments use the [Qwen/Qwen3-4B-Instruct-2507](https://huggingface.co/Qwen/Qwen3-4B-Instruct-2507).
...
<p align="center">
  <img src="../assets/countdown_rl_lr_rank_final_reward.png" alt="countdown_rl_final_reward_vs_lr" width="720">
</p>
